...
const  validateCoupanInput = require('../../validation/coupan')
const password = require("../../config/keys").password;


// Load User model eeded
...
router.get("/allCostomer", (req, res) => {
  user.exec(function(err, data){
    ...
    const filtered = data.filter(element => element.role !== "admin");
  ...
  router.post('/getProduct',   passport.authenticate('jwt', { session: false }),  function(req, res) {
   ...
   await     Product.findOneAndUpdate({  _id:req.body.id  } , {$set: { "img1": req.body.img1  }},function(err, user) {
       ...
       router.post('/img2Change',   passport.authenticate('jwt', { session: false }), async  function(req, res) {
        ...
        await     Product.findOneAndUpdate({  _id: req.body.id ,} , {$set: { "productName": req.body.productName ,  "price": req.body.price , "decription": req.body.description , "stocks": req.body.stocks  }},function(err, user) {
